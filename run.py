...
def splitData(dir, ratio):
	#dir is the name of the directory, can be relative
	#ratio is the ratio of training data to test data 0 < ratio < 1
	...
	for imgclass in classes:
		...
		data = [name for name in os.listdir(classDir)
				if os.path.isfile(os.path.join(classDir, name))]

		# os.mkdir(os.path.join('.', dir, "train"))
		# os.mkdir(os.path.join('.', dir, "test"))
		train_dir = os.path.join('.', dir, "train", imgclass)
		test_dir = os.path.join('.', dir, "test", imgclass)
		...
		ravg = 0
		for d in data:
			src = os.path.join(classDir, d)
			if random.random() <= ratio:
				dst = os.path.join('.', dir, "train", imgclass, d)
				ravg+=1
			else:
				dst = os.path.join('.', dir, "test", imgclass)
